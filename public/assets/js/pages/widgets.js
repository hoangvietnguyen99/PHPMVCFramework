"use strict";

// Class definition
var KTWidgets = function () {
    function getElement(data) {
        let publishDate = new Date(data.createdDate.date + data.createdDate.timezone);

        const element = document.createElement('div');
        element.innerHTML = `<div class="card card-custom gutter-b">
                            <div class="card-body d-flex flex-row justify-content-around">
                                <!--begin::Action-->
                                <div class="d-flex align-items-center justify-content-between mr-5">
                                    <a href="#"
                                       class="btn btn-text-primary btn-icon-primary btn-sm bg-light-primary rounded font-weight-bolder font-size-sm p-2 mr-1">
																<span class="svg-icon svg-icon-md svg-icon-primary-25 pr-2"><!--begin::Svg Icon | path:C:\wamp64\www\keenthemes\legacy\metronic\theme\html\demo9\dist/../src/media/svg/icons\Navigation\Angle-double-up.svg--><svg
                                                                            xmlns="http://www.w3.org/2000/svg"
                                                                            xmlns:xlink="http://www.w3.org/1999/xlink"
                                                                            width="24px" height="24px"
                                                                            viewBox="0 0 24 24" version="1.1">
    <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
        <polygon points="0 0 24 0 24 24 0 24"/>
        <path d="M8.2928955,10.2071068 C7.90237121,9.81658249 7.90237121,9.18341751 8.2928955,8.79289322 C8.6834198,8.40236893 9.31658478,8.40236893 9.70710907,8.79289322 L15.7071091,14.7928932 C16.085688,15.1714722 16.0989336,15.7810586 15.7371564,16.1757246 L10.2371564,22.1757246 C9.86396402,22.5828436 9.23139665,22.6103465 8.82427766,22.2371541 C8.41715867,21.8639617 8.38965574,21.2313944 8.76284815,20.8242754 L13.6158645,15.5300757 L8.2928955,10.2071068 Z"
              fill="#000000" fill-rule="nonzero"
              transform="translate(12.000003, 15.500003) scale(-1, 1) rotate(-90.000000) translate(-12.000003, -15.500003) "/>
        <path d="M6.70710678,12.2071104 C6.31658249,12.5976347 5.68341751,12.5976347 5.29289322,12.2071104 C4.90236893,11.8165861 4.90236893,11.1834211 5.29289322,10.7928968 L11.2928932,4.79289682 C11.6714722,4.41431789 12.2810586,4.40107226 12.6757246,4.76284946 L18.6757246,10.2628495 C19.0828436,10.6360419 19.1103465,11.2686092 18.7371541,11.6757282 C18.3639617,12.0828472 17.7313944,12.1103502 17.3242754,11.7371577 L12.0300757,6.88414142 L6.70710678,12.2071104 Z"
              fill="#000000" fill-rule="nonzero" opacity="0.3"
              transform="translate(12.000003, 8.500003) scale(-1, 1) rotate(-360.000000) translate(-12.000003, -8.500003) "/>
    </g>
</svg><!--end::Svg Icon--></span>${data.totalLikes}</a>
                                    <a href="#"
                                       class="btn btn-icon-danger btn-text-danger bg-light-danger btn-sm rounded font-weight-bolder font-size-sm p-2 mr-1">
																<span class="svg-icon svg-icon-md svg-icon-25 pr-2"><!--begin::Svg Icon | path:C:\wamp64\www\keenthemes\legacy\metronic\theme\html\demo9\dist/../src/media/svg/icons\Navigation\Angle-double-down.svg--><svg
                                                                            xmlns="http://www.w3.org/2000/svg"
                                                                            xmlns:xlink="http://www.w3.org/1999/xlink"
                                                                            width="24px" height="24px"
                                                                            viewBox="0 0 24 24" version="1.1">
    <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
        <polygon points="0 0 24 0 24 24 0 24"/>
        <path d="M8.2928955,3.20710089 C7.90237121,2.8165766 7.90237121,2.18341162 8.2928955,1.79288733 C8.6834198,1.40236304 9.31658478,1.40236304 9.70710907,1.79288733 L15.7071091,7.79288733 C16.085688,8.17146626 16.0989336,8.7810527 15.7371564,9.17571874 L10.2371564,15.1757187 C9.86396402,15.5828377 9.23139665,15.6103407 8.82427766,15.2371482 C8.41715867,14.8639558 8.38965574,14.2313885 8.76284815,13.8242695 L13.6158645,8.53006986 L8.2928955,3.20710089 Z"
              fill="#000000" fill-rule="nonzero"
              transform="translate(12.000003, 8.499997) scale(-1, -1) rotate(-90.000000) translate(-12.000003, -8.499997) "/>
        <path d="M6.70710678,19.2071045 C6.31658249,19.5976288 5.68341751,19.5976288 5.29289322,19.2071045 C4.90236893,18.8165802 4.90236893,18.1834152 5.29289322,17.7928909 L11.2928932,11.7928909 C11.6714722,11.414312 12.2810586,11.4010664 12.6757246,11.7628436 L18.6757246,17.2628436 C19.0828436,17.636036 19.1103465,18.2686034 18.7371541,18.6757223 C18.3639617,19.0828413 17.7313944,19.1103443 17.3242754,18.7371519 L12.0300757,13.8841355 L6.70710678,19.2071045 Z"
              fill="#000000" fill-rule="nonzero" opacity="0.3"
              transform="translate(12.000003, 15.499997) scale(-1, -1) rotate(-360.000000) translate(-12.000003, -15.499997) "/>
    </g>
</svg><!--end::Svg Icon--></span>${data.totalDislikes}</a>
                                    <a href="#"
                                       class="btn btn-text-success btn-icon-success bg-light-success btn-sm rounded font-weight-bolder font-size-sm p-2">
																<span class="svg-icon svg-icon-md svg-icon-25 pr-2">
																	<!--begin::Svg Icon | path:assets/media/svg/icons/Communication/Group-chat.svg-->
																	<svg xmlns="http://www.w3.org/2000/svg"
                                                                         xmlns:xlink="http://www.w3.org/1999/xlink"
                                                                         width="24px" height="24px" viewBox="0 0 24 24"
                                                                         version="1.1">
																		<g stroke="none" stroke-width="1" fill="none"
                                                                           fill-rule="evenodd">
																			<rect x="0" y="0" width="24" height="24"/>
																			<path d="M16,15.6315789 L16,12 C16,10.3431458 14.6568542,9 13,9 L6.16183229,9 L6.16183229,5.52631579 C6.16183229,4.13107011 7.29290239,3 8.68814808,3 L20.4776218,3 C21.8728674,3 23.0039375,4.13107011 23.0039375,5.52631579 L23.0039375,13.1052632 L23.0206157,17.786793 C23.0215995,18.0629336 22.7985408,18.2875874 22.5224001,18.2885711 C22.3891754,18.2890457 22.2612702,18.2363324 22.1670655,18.1421277 L19.6565168,15.6315789 L16,15.6315789 Z"
                                                                                  fill="#000000"/>
																			<path d="M1.98505595,18 L1.98505595,13 C1.98505595,11.8954305 2.88048645,11 3.98505595,11 L11.9850559,11 C13.0896254,11 13.9850559,11.8954305 13.9850559,13 L13.9850559,18 C13.9850559,19.1045695 13.0896254,20 11.9850559,20 L4.10078614,20 L2.85693427,21.1905292 C2.65744295,21.3814685 2.34093638,21.3745358 2.14999706,21.1750444 C2.06092565,21.0819836 2.01120804,20.958136 2.01120804,20.8293182 L2.01120804,18.32426 C1.99400175,18.2187196 1.98505595,18.1104045 1.98505595,18 Z M6.5,14 C6.22385763,14 6,14.2238576 6,14.5 C6,14.7761424 6.22385763,15 6.5,15 L11.5,15 C11.7761424,15 12,14.7761424 12,14.5 C12,14.2238576 11.7761424,14 11.5,14 L6.5,14 Z M9.5,16 C9.22385763,16 9,16.2238576 9,16.5 C9,16.7761424 9.22385763,17 9.5,17 L11.5,17 C11.7761424,17 12,16.7761424 12,16.5 C12,16.2238576 11.7761424,16 11.5,16 L9.5,16 Z"
                                                                                  fill="#000000" opacity="0.3"/>
																		</g>
																	</svg>
                                                                    <!--end::Svg Icon-->
																</span>${data.totalAnswers}</a>
                                </div>
                                <!--end::Action-->
                                <div class="flex-grow-1">
                                    <!--begin::Header-->
                                    <div class="d-flex align-items-center flex-grow-1">
                                        <!--begin::Info-->
                                        <div class="d-flex flex-column flex-grow-1">
                                            <a href="/questions?id=${data['_id']['$oid']}"
                                               class="text-dark-75 text-hover-primary mb-1 font-size-lg font-weight-boldest">${data.title}</a>
                                            <span class="text-muted font-weight-bolder">Categories: ${data.categories.map(category => `<a href="/questions?category=${category['_id']['$oid']}">${category.name}</a>`).join(', ')}</span>
                                            <span class="text-muted font-weight-bolder">Tags: ${data.tags.map(tag => `<a href="/questions?tag=${tag['_id']['$oid']}">${tag.name}</a>`).join(', ')}</span>
                                        </div>
                                        <!--end::Info-->
                                        <div class="d-flex flex-column align-items-end ml-2">
                                            <a href="/account?id=${data.author['_id']['$oid']}"
                                               class="text-dark-75 text-hover-success mb-1 font-size-lg font-weight-bolder">${data.author.name}</a>
                                            <span class="text-muted font-weight-bold">asked ${moment(publishDate).fromNow()}</span>
                                        </div>
                                        <!--begin::Symbol-->
                                        <div class="symbol symbol-40 symbol-light-success ml-2">
																<span class="symbol-label">
																	<img src="${data.author.imgPath}" class="h-75 align-self-end"
                                                                         alt=""/>
																</span>
                                        </div>
                                        <!--end::Symbol-->
                                    </div>
                                    <!--end::Header-->
                                </div>
                            </div>
                        </div>`;

        return element;
    }


    // Education Show More Demo
    var _initEducationShowMoreBtn = function() {
        var el = KTUtil.getById('kt_app_education_more_feeds_btn');
        var container = KTUtil.getById('questions_container');

        if (!el && !container) {
            return;
        }

        fetch('/api/questions?' + new URLSearchParams(location.search))
            .then(response => {
                if (response.ok) {
                    const contentType = response.headers.get('content-type');
                    if (!contentType || !contentType.includes('application/json')) {
                        throw new TypeError("Oops, we haven't got JSON!");
                    }
                    return response.json();
                }
            })
            .then(body => {
                let item;

                for (let i = 0; i < body.length; i++) {
                    let data = body[i];

                    let element = getElement(data);

                    if (!item) item = element;
                    container.appendChild(element);
                }

                KTUtil.scrollTo(item);
            });

        KTUtil.addEvent(el, 'click', function(e) {
            KTUtil.btnWait(el, 'spinner spinner-right spinner-white pr-15', 'Please wait...', true);

            fetch('/api/questions?' + new URLSearchParams(location.search) + '&' + new URLSearchParams({
                // limit: 5,
                // sort: 'createdAt',
                // order: -1,
                skip: container.childElementCount
            }))
                .then(response => {
                    if (response.ok) {
                        const contentType = response.headers.get('content-type');
                        if (!contentType || !contentType.includes('application/json')) {
                            throw new TypeError("Oops, we haven't got JSON!");
                        }
                        return response.json();
                    }
                })
                .then(body => {
                    KTUtil.btnRelease(el);

                    let item;

                    if (!body.length) {
                        Swal.fire({
                            title: "Error!",
                            text: "No more question found!",
                            icon: "error",
                            buttonsStyling: false,
                            confirmButtonText: "Ok!",
                            customClass: {
                                confirmButton: "btn btn-danger"
                            }
                        });
                        return;
                    }

                    for (let i = 0; i < body.length; i++) {
                        let data = body[i];

                        let element = getElement(data);

                        if (!item) item = element;
                        container.appendChild(element);
                    }

                    KTUtil.scrollTo(item);
                });
        });
    }

    // Public methods
    return {
        init: function () {
            // Education App
            _initEducationShowMoreBtn();
        }
    }
}();

jQuery(document).ready(function () {

    KTWidgets.init();
});
